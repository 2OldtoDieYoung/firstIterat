...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	avarageSpeedM := meanSpeed(action, duration) * kmhInMsec
	heightM := height / cmInM
	calories := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(avarageSpeedM, 2)/heightM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
